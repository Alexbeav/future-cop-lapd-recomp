...
#
# Writes: dist/futurecoplapd-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix futurecoplapd \
  --exe-name Future_Cop_LAPD \
  --omit-openbios \
  --display-name "Future Cop: L.A.P.D." \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Future Cop: L.A.P.D. (Europe) disc" \
  --runtime-dir mods \
